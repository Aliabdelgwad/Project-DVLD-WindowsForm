...
namespace DVLD.Applications
{
    ...
    public partial class frmAddUpdateLocalDrivingLicesnseApplication: Form
    {
        ...
        private void cbLicenseClass_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                DateTime AgeOfPerson = clsPerson.Find(_SelectedPersonID).DateOfBirth;
                DateTime MinimumAllowedAge = DateTime.Now.AddYears(-clsLicenseClass.Find(cbLicenseClass.Text).MinimumAllowedAge);

                if ( AgeOfPerson >= MinimumAllowedAge)
                {
                     e.Cancel = true;
                    errorProvider1.SetError(cbLicenseClass, "Age Of Person Mainmam from Requered!");
                }
                else
                {
